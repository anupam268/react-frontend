...
function createData(name, calories, fat, carbs, protein, detail, url, status) {
  return { name, calories, fat, carbs, protein, detail, url, status };
}
...
const SimpleTable = () => {
  ...
  const handleSwitchChange = (row) => {
    const updatedRows = rows.map((r) =>
      r.id === row.id
        ? { ...r, status: r.status === "neutral" ? "correct" : "incorrect" }
        : r
    );

    // Log the updated row if needed
    console.log(
      "Updated Row:",
      updatedRows.find((r) => r.id === row.id)
    );

    // If you want to update the state for re-rendering
    setRows(updatedRows);
  };
                    ...
                    <TableCell key={headCell.id} component="th" scope="row">
                      {/* {headCell.id !== "actions" ? (
                        ...
                        <Button
                          variant="contained"
                          color="primary"
                          onClick={() => window.open(row.actions, "_blank")}
                        >
                          Open URL
                        </Button>
                      )} */}
